...
PLOT_DIR = r"C:\Users\julie\OneDrive - Imperial College London\data for binary annotations"
OUTPUT_CSV = r"C:\Users\julie\OneDrive - Imperial College London\binary output\FSCV_Labels_Nov.csv"
BACKUP_DIR = r"C:\Users\julie\OneDrive - Imperial College London\binary output\label_backups"
FSCV_HZ = 10                       # Sampling rate (frames per second)

# Label types: {id: (name, color)}
LABELS = {0: ("Nothing", "blue"), 1: ("Serotonin", "red"), 2: ("Uncertain", "orange")}
...
class PLOT_SETTINGS:
    """Custom colormap settings for FSCV data visualisation."""
    
    def __init__(self):
        # Custom colormap for FSCV data visualisation
    ...
    def get_norm(self, data, clim=None, vmax=None):
        """
        Color normalisation for the FSCV color plot.
...
class FSCVLabeler:
    ...
    def draw(self):
        ...
        ctrl = "DELETE: 1-9=del label, D=all, ESC=cancel" if self.del_mode else "[0/1/2]=Label [X]=Del [D]=All [B]=BG [J]=Jump [Q]=Quit"
    ...
    def on_key(self, event):
        ...
        # Normal mode
        if k in '012': self.add_label(int(k)); self.goto(self.idx + 1); self.draw()
...
def main():
    ...
    print("\n[Enter]=All [0/1/2]=Review label [u]=Unlabeled [m]=Multiple")
    
    # Get mode choice
    choice = input("Mode: ").strip().lower()
    review = int(choice) if choice in ['0', '1', '2'] else None
    ...
    print("\nControls: Drag=Select | 0/1/2=Label | ←/→=Nav | X=Delete mode | D=Delete all | B=BG | J=Jump | Q=Quit\n")
    app.run(start_idx)
